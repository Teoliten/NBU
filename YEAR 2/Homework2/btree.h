...
public:
    ...
    BTree<TKey>& operator=(const BTree<TKey>& other); // overloaded assignment operator
...
template <typename TKey>
BTree<TKey>::BTree(const BTree<TKey> &other) : ptr_root(nullptr), numb(0)
{
  copyTree(other.ptr_root, ptr_root);
}// copy constructor

template <typename TKey>
void BTree<TKey>::copyTree(const Node *sourceNode, Node *&targetNode)
{
  if (sourceNode)
  {
    targetNode = new Node(sourceNode->key);

    copyTree(sourceNode->ptr_left, targetNode->ptr_left);
    copyTree(sourceNode->ptr_right, targetNode->ptr_right);

    if (targetNode->ptr_left)
    {
        targetNode->ptr_left->ptr_prnt = targetNode;
    }
    if (targetNode->ptr_right)
    {
        targetNode->ptr_right->ptr_prnt = targetNode;
    }
...
template <typename TKey>
BTree<TKey>& BTree<TKey>::operator=(const BTree<TKey>& other)
{
    if (this != &other) // avoid self assignment
    {
        deleteTree(ptr_root);               // delete old
        copyTree(other.ptr_root, ptr_root); // copy from new one into old
    }
    return *this;
} // operator =
